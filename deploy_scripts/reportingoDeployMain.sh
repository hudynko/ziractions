...
SRC_DIR="/docker/projects/reportingo/reportingo_production_defaults"
...
# Check if quick deploy mode
  ...
  echo "🔧 Full deployment mode - preparing Reportingo production deployment for branch: $BRANCH"
...
if [ "$BUILD_TYPE" = "backend" ]; then
    ...
    docker rmi "10.0.0.2:5000/reportingobackendproduction:$version" 2>/dev/null || true
    
    # Remove from registry (optional but ensures fresh push)
    curl -X DELETE "http://10.0.0.2:5000/v2/reportingobackendproduction/manifests/latest" 2>/dev/null || true
    if ! docker build --no-cache -t "10.0.0.2:5000/reportingobackendproduction:$version" -f DockerfileBackEnd . ; then
        echo "❌ Backend build failed! Stopping deployment."
        exit 1
    fi
    docker push "10.0.0.2:5000/reportingobackendproduction:$version" > /dev/null 2>&1
    
    echo "🚀 Backend-only deployment - updating service..."
    docker service update --image --force "10.0.0.2:5000/reportingobackendproduction:$version" reportingoproduction_reportingo-production-api
    echo "✅ Backend service updated successfully!"
    exit 0
    
elif [ "$BUILD_TYPE" = "frontend" ]; then
    echo "🔧 Building frontend only..."
    if ! docker build --no-cache -t "10.0.0.2:5000/reportingofrontendproduction:$version" -f DockerfileFrontEnd .; then
        echo "❌ Frontend build failed! Stopping deployment."
        exit 1
    fi
    docker push "10.0.0.2:5000/reportingofrontendproduction:$version" > /dev/null 2>&1
    
    echo "🚀 Frontend-only deployment - updating service..."
    docker service update --image --force "10.0.0.2:5000/reportingofrontendproduction:$version" reportingoproduction_reportingo-production-frontend
    ...
    if [ "$QUICK_DEPLOY" != "full" ]; then
        ...
        docker service update --force --image "10.0.0.2:5000/reportingobackendproduction:$version" reportingoproduction_reportingo-production-api
        docker service update --force --image "10.0.0.2:5000/reportingofrontendproduction:$version" reportingoproduction_reportingo-production-frontend
...
STACK_NAME="reportingoproduction"
